...
const TRIANGLE_BYTES_SIZE: usize = 48;
...
fn bytes_to_f32(b: &[u8; 4]) -> f32 {
    return f32::from_be_bytes(*b);
}

#[allow(non_snake_case)] fn bytes_to_point3D(b: &[u8; 12]) -> Point3D {
    return Point3D {
...
fn bytes_to_triangle(b: &[u8; TRIANGLE_BYTES_SIZE]) -> stl_triangle::TriangleSTL {
    let point0 = bytes_to_point3D(b[..12].try_into().expect("Invalid file format (point 0)"));
    let point1 = bytes_to_point3D(b[12..24].try_into().expect("Invalid file format (point 1)"));
    let point2 = bytes_to_point3D(b[24..36].try_into().expect("Invalid file format (point 2)"));
    let normal = bytes_to_point3D(b[36..48].try_into().expect("Invalid file format (normal)"));
